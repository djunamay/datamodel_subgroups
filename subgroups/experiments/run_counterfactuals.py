...
def run_counterfactuals(cf_inputs, counterfactual_estimator, n_iter=100, n_clusters=2, random_state=None, shuffle_rng=None, model_rng=None):
    ...
    for name, mat in cf_inputs:
        partition_storage = PartitionStorageBase(matrix=mat, partitioner=SpectralClustering(n_clusters=n_clusters, affinity='precomputed', assign_labels='kmeans', random_state=np.random.RandomState(random_state.bit_generator)))
