...
from ..classifiers import model_factory_fn, SklearnClassifier
...
class TrainClassifiersArgs:
    ...
    model_factory : model_factory_fn
        Builds fresh classifiers instances.
    n_models : int
        Number of classifiers (i.e., training splits) to run.
    random_generator : RandomGeneratorTC
    ...
    mask_factory: mask_factory_fn = chz.field(default=None, doc='Factory for generating masks. This will be used for training the classifiers.')
    model_factory: model_factory_fn = chz.field(default=None, doc='Factory for creating classifiers. This will be used for training the classifiers.')
    n_models: int = chz.field(default=None, doc='Number of classifiers to build from ModelFactory. Each model will be trained on a different mask from MaskFactory.')
    ...
    random_generator: RandomGeneratorTC = chz.field(default=None, doc='Random generator for training the classifiers.')
    npcs: int = chz.field(default=None, doc='Number of PCs to use for the classifiers.')
    feature_selector: select_features_fn = chz.field(default=None, doc='Feature selector for the classifiers.')
...
def fit_single_classifier(features: NDArray[np.float32], labels: NDArray[bool], mask: NDArray[bool], model: SklearnClassifier, shuffle_rng: np.random.Generator):
    """
    Train a single classifiers and compute margins and test accuracy.
...
def run_training_batch(args: TrainClassifiersArgs, batch_starter_seed: int):
    ...
    args : TrainClassifiersArgs
        ...
        mask factory, model factory, number of classifiers, and storage options.
    ...
    """
    ds = args.dataset
    feature_indices = args.feature_selector(args.npcs)
    ...
    for i, mask in tqdm(enumerate(storage.masks), desc='Training classifiers', total=args.n_models):
        ...
        clf = args.model_factory(rng=build_model_rngs_children[i])
...
# python -m subgroups.experiments.train_classifiers dataset=subgroups.datasets.registry:gtex mask_factory=subgroups.datasamplers.mask_generators:fixed_alpha_mask_factory model_factory=subgroups.classifiers.model_factory_xgboost n_models=10
